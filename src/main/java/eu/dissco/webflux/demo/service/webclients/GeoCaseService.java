...
public class GeoCaseService implements WebClientInterface{
    ...
    public Stream<DarwinCore> retrieveData() {
        ...
        return webClient.get().uri(uri + "&start=0").retrieve().bodyToFlux(JsonNode.class).expand(
                response -> {
                    ...
                    if (currentPosition + properties.getItemsPerRequest() <= total) {
                        ...
                        return webClient.get().uri(uri + "&start=" + currentPosition).retrieve().bodyToFlux(JsonNode.class);
                    }
                    return Flux.empty();
    ...
    private List<DarwinCore> mapToDarwin(JsonNode response) {
        ...
        items.forEach(item -> {
            var darwin = DarwinCore.builder()
                    ...
                    .scientificName(item.has("fullscientificname") ? item.get("fullscientificname").asText() : null)
